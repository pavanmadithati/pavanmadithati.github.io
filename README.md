# Pavan Madithati
**Enterprise Security Architect | Cybersecurity Leader | Zero Trust | Cloud, Network, App & Data Security**

Atlanta, GA | Open to Remote / Relocation  
📧 pavan.madithati@hotmail.com  
🔗 https://linkedin.com/in/pavanmadithati  

---

## Professional Summary

Enterprise Security Architect with 13+ years of experience designing and leading enterprise cybersecurity programs across network, cloud, data, identity, and SOC domains. Proven track record delivering Zero Trust, SASE/SSE, CNAPP, and SOC modernization initiatives for environments supporting 30,000+ users and 700+ locations. Expert in AWS and Azure security, enterprise network architecture, detection engineering, and DevSecOps automation. Trusted advisor to executives, driving risk-based decisions, multi-year security roadmaps, and measurable risk reduction.

---

## Core Skills

### Security Architecture & Strategy
Zero Trust | Defense-in-Depth | Security-by-Design | Enterprise Security Architecture | Threat Modeling | Risk-Based Design | Security Roadmaps | Reference Architectures

---

### Network Security
NGFW: Palo Alto, FortiGate, Cisco Firepower, Check Point  
Firewall Policy Design | SSL Ecryption/Decryption | App-ID | User-ID  
Network Segmentation (Macro/Micro) | East-West / North-South Controls  
VPN (IPSec, SSL) | Remote Access | Reverse Proxy  
Cisco ACI | Cisco Umbrella | Cisco Catalyst Center | Cisco Secure X | Cisco ThousandEyes  
DDoS Protection (Arbor)  
Load Balancing: F5 LTM/GTM/APM | FortiADC | WAF | L4-L7 | Netscalar ADC  
Routing: BGP, OSPF, EIGRP | MPLS | DMVPN  
Switching: Nexus, Catalyst | Data Center Networking | VLAN | STP | VxLAN  
NAC: Cisco ISE (802.1X, EAP-TLS, Posture, Profiling) | Aruba ClearPass   
SASE / SSE (Zscalar, Palo Alto, Netskope) | SD-WAN | ZTNA | SWG | CASB | FWaaS 

---

### Cloud Security
AWS | Azure | Hybrid & Multi-Cloud Architecture  
VPC / VNets | Subnets | Routing | Security Groups / NSGs / NACL's
Cloud Guardrails | Landing Zones | ExpressRoute / Direct-Connect | CDN | Site-to-Site VPN  
Identity-Aware Access | Shared Responsibility Models

---

### Cloud-Native Protection
CNAPP (Prisma Cloud) | CSPM | CWPP | CIEM | DSPM | KSPM | ASPM  
Container Security | Kubernetes | Docker  
Runtime Protection | Cloud Vulnerability & Exposure Management

---

### Data Security
Enterprise DLP Programs | Data Classification | Data Labeling 
Encryption & Key Management (Azure Key Vault, AWS KMS)  
Secure Data Flows | Data Exposure Risk Reduction  
Privacy & Regulatory Alignment

---

### Application Security (Shift-Left)
Secure SDLC | OWASP Top 10  
SAST | DAST | SCA
CI/CD Security Integration | Pipeline Hardening  
Terraform Security | Infrastructure Security Controls  
API Security | OAuth2 | OIDC | SAML  
Wiz | Prisma Cloud Application Security | Synk

---

### Identity & Access Security
IAM | IGA | PAM  
SSO | MFA | Conditional Access  
Azure AD / Entra ID | Okta | SailPoint  
PKI Architecture & Certificate Lifecycle Management  

---

### SOC & Detection Engineering
Microsoft Sentinel | Splunk | Defender XDR | Cortex XSOAR  
Threat Hunting | Incident Response | Purple Teaming  
MITRE ATT&CK Mapping | Detection Use Case Engineering  
SOC Metrics: MTTD | MTTR | Alert Fidelity

---

### Vulnerability & Exposure Management
Rapid7 | Tenable Nessus  
Penetration Testing Coordination  
Risk-Based Remediation Programs

---

### Automation & DevSecOps
Terraform | Infrastructure-as-Code | Policy-as-Code  
Python | PowerShell  
CI/CD Integration (GitHub, Azure DevOps)  
API-Driven Security | Automated Remediation | SOAR

---

### AI Security
AI/ML Risk Awareness | Data Exposure Controls for AI  
Model Access Governance | Secure AI Usage Guardrails

---

### Governance & Compliance
NIST CSF | ISO 27001 | SOC2 | PCI DSS | HIPAA | GDPR  
Risk Assessments | Audit Readiness | Security Metrics | Executive Reporting

---

## Certifications

CISSP – Certified Information Systems Security Professional  
CISM – Certified Information Security Manager  
CRISC – Certified in Risk and Information Systems Control  
Microsoft Azure Cybersecurity Architect  
AWS Certified Solutions Architect – Associate  
CCNP – Cisco Certified Network Professional  
PCNSA – Palo Alto Networks Certified Network Security Administrator  
ITIL v4  
Splunk Certified User  

---
## Professional Experience

### Government of Alberta — Senior Enterprise Security Architect

2023 – Present | Edmonton, AB

* Designed enterprise security architecture supporting **700+ sites and 25,000+ users**, improving overall risk posture by **40%**.
* Implemented **Zero Trust segmentation and identity-aware access**, reducing lateral movement risk by **50%**.
* Led deployment of **Prisma Cloud (CNAPP)**, reducing critical cloud misconfigurations by **55%**.
* Established **AWS and Azure security guardrails** using Infrastructure-as-Code, improving compliance visibility across **1,000+ cloud resources**.
* Modernized SOC capabilities by implementing advanced **Sentinel analytics and automation**, reducing incident response time by **45%**.
* Integrated security controls into CI/CD pipelines, preventing high-risk deployments and reducing configuration drift by **35%**.
* Implemented **Arbor-based DDoS protection** with BGP traffic control, reducing attack impact by **70%**.
* Developed executive security dashboards and risk metrics to support enterprise decision-making.

---

### Government of Alberta — Enterprise Security Architect Consultant

2021 – 2023

* Led **SASE and secure remote access initiatives**, improving secure user access for **10,000+ remote users**.
* Designed **hybrid cloud network security architecture** integrating on-prem, AWS, and Azure environments.
* Implemented **Terraform-based security deployments**, reducing manual provisioning effort by **40%**.
* Built **enterprise detection use cases aligned to MITRE ATT&CK**, improving threat visibility by **30%**.
* Conducted enterprise security architecture reviews for major digital transformation programs.

---

### Government of Alberta — Senior Security Consultant

2018 – 2021

* Implemented NGFW modernization across multiple environments, improving threat blocking effectiveness by **45%**.
* Deployed **Cisco ISE and network access controls**, strengthening device posture enforcement across enterprise networks.
* Supported SOC operations with threat hunting and incident response, reducing dwell time by **35%**.
* Designed secure connectivity models for SaaS and cloud onboarding initiatives.

---

### AFSC — Network & Cloud Security Consultant  2015 – 2018

* Designed secure enterprise network architecture across **50+ locations**, improving availability to **99.99%**.
* Implemented **F5 load balancing and WAF protections** for critical applications.
* Supported cloud migration by designing secure **VPN and segmentation architectures** for AWS and Azure.
* Automated firewall policy management and reporting, reducing operational effort by **25%**.

---

### IT Partners Inc  
**Information Security Consultant** | 2014 – 2015

- Delivered security architecture and access control solutions for enterprise and public sector clients.
- Conducted security assessments and gap analyses with prioritized remediation roadmaps.
- Implemented NGFW, IDS/IPS, and network segmentation architectures.
- Established secure baselines and access governance for managed service onboarding.

---

## Education
Master’s in Electrical & Computer Engineering,Concordia University, Canada.	     2012 - 2014

Bachelor of Technology in Electronics & Communication Engineering, JNTU University, India.   2007 – 2011

---

## Key Achievements

* Secured environments supporting **25,000+ users and 700+ locations**
* Reduced critical cloud misconfigurations by **55%**
* Improved incident response time by **45%**
* Achieved **99.99% availability** for mission-critical services
* Reduced lateral movement exposure by **50%**
